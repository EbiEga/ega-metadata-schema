...
def keep_data_prop(new_json:dict) -> dict:
    ...
    if not isinstance(new_json, dict):
        raise ValueError("new_json must be a loaded JSON object (i.e. dictionary).")
    if not data_property_name in new_json:
        raise ValueError(f"new_json does not contain '{data_property_name}' property at root level")
...
def load_json(json_filepath:str) -> dict:
    ...
    if not os.path.isfile(json_filepath) or not file_extension == ".json":
        raise ValueError(f"File '{json_filepath}' does not exist, is not a file, or is not a JSON file")
...
class CreateObjectSet:
    ...
    def empty_all_properties(self):
        ...
        self.template_json_obj = empty_json_values(self.template_json_obj, root_level=True)
        
    def add_new_json(self, new_json: Union[str,dict]):
        """
        ...
        if isinstance(new_json, str):
            ...
            raise ValueError(f"New JSON to be added '{new_json}' must be either the filepath to a JSON document or the loaded dictionary of a JSON")
    ...
    def count_n_objects(self):
        ...
        """
        n_of_objects = len(self.template_json_obj[self.data_name][self.objectArray_name])
        return n_of_objects
        
    def save_json(self, output_filepath:str = "new_object-set.json", add_suffix:bool = False, overwrite:bool = False):
        """
        ...
        if os.path.isfile(output_filepath) and not overwrite:
            warnings.warn(f"File '{output_filepath}' already exists and 'overwrite' was set to False. Choose a different filename or set overwrite to True.", UserWarning)
            return
